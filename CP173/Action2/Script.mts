	...
	If ((ucase(Motivo) = "CAEQ_EQUIPO") or (ucase(Motivo) = "CAEQ_CAPL")) Then
		...
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").Exist Then
			...
			If varmens="No se cuenta con stock del equipo HUAWEI Y360 BLANCO. Por favor seleccionar uno distinto." Then
				...
				wait 1
			End If
			If varmens = "La respuesta de SAP est?? vac??a para la Comprobaci??n de disponibilidad." Then
				wait 1
				JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "MensajeError.png", True
				imagenToWord "Mensaje",RutaEvidencias() & "MensajeError.png" 
				JavaWindow("Ejecutivo de interacción").JavaDialog("Mensaje").JavaButton("OK").Click
				ExitActionIteration
			End If
		End If
...
RunAction "WIC2", oneIteration
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").JavaButton("Enviar orden").Click @@ hightlight id_;_12430702_;_script infofile_;_ZIP::ssf2.xml_;_
		...
		t=0
		While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Orden 836795A").JavaEdit("TextAreaNative$1").Exist or JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").Exist = False
			Wait 1
			t = t + 1
			If (t >= 180) Then
				DataTable("s_Resultado", dtlocalSheet) = "Fallido"
		...
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").Exist = true Then
			Dim filass, text1, j1
			filass = JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaTable("SearchJTable").GetROProperty("rows")
			filass = filas-1
			For Iterator = 0 To filass Step 1
				text1 = JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaTable("SearchJTable").GetCellData("#"&Iterator, "#1")
				j1 = Instr(1,text,"Por favor considere que se agrego un cargo de penalidad a la acción de orden")
				If j1 <> "0"  Then
					JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "MensajeValidacion.png", True
					imagenToWord "Mensaje de Validación",RutaEvidencias() & "MensajeValidacion.png"
					JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaButton("Aceptar").Click
			...
			Do
				If (DataTable("s_Detalle", dtLocalSheet)="Por favor rellenar todas las identificaciones de equipos") or (DataTable("s_Detalle", dtLocalSheet)<>"Por favor rellenar todas las identificaciones de equipos") Then
					...
					Wend
						...
						If  DataTable("Motivo", dtLocalSheet)="CAEQ_EQUIPO" or  DataTable("Motivo", dtLocalSheet)="CAEQ_CAPL" Then
...
Sub OrdenCerrada()
	...
	Set shell = CreateObject("Wscript.Shell") 
	shell.SendKeys "{ENTER}"
	While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").JavaEdit("ID de acción de orden:").Exist = False
		wait 1
	Wend
	Dim g
	g = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").JavaEdit("ID de acción de orden:").GetROProperty("text")
	While g = ""
		wait 1
		g = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").JavaEdit("ID de acción de orden:").GetROProperty("text")
	Wend
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").JavaTab("Nombre del cliente:").Select "Actividad"
	While JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").JavaTable("SearchJTable").Exist = False
		wait 1
	Wend
	wait 3
	Set shell = CreateObject("Wscript.Shell") 
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Ver acción de orden: 1092060A").Close
	wait 1
