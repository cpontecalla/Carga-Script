...
Sub EncontrarAcuerdoComercial()
	...
	wait 1
		...
		Fila = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de cliente empresarial").JavaTable("SearchJTable").GetROProperty("rows")
		Fila = Fila-1
		For i=0 to Fila Step 1
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de cliente empresarial").JavaTable("SearchJTable").ClickCell i, 1
'				
				...
				'Valor = Left(Valor,21)
				Valor2 = DataTable("e_Grupo_Negocio", dtLocalSheet)
		If Valor = Valor2 Then
			JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de cliente empresarial").JavaTable("SearchJTable").ActivateRow "#"&i
...
Sub RecursosAlta()
	...
	wait 2
		...
		Select Case ucase(DataTable("e_Financiamiento", dtLocalSheet))
			Case "12"
				...
				Call Carga()
				wait 2
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").CaptureBitmap RutaEvidencias() &"Financiamineto12.png", True
				imagenToWord "Financiamiento calculado:", RutaEvidencias() &"Financiamineto12.png"
			Case "18"
			...
			Case "NO"
				...
				Call Carga()
				wait 2
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Configuración").CaptureBitmap RutaEvidencias() &"Contado.png", True
				imagenToWord "Monto calculado:", RutaEvidencias() &"Contado.png"
		End Select
	...
	Select Case str_metodo_entrega
		...
		Case "Delivery"
				...
				wait 1
					...
					Wend
				wait 1
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar dirección de_2").JavaList("ComboBoxNative$1").Select DataTable("e_Tipo_Docu",dtLocalSheet)			
				wait 1
				JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar dirección de_2").JavaEdit("TextFieldNative$1").Set DataTable("e_Cliente_Direc_Envio", dtLocalSheet)
				wait 1
...
Sub GeneracionOrden()
	...
	If JavaWindow("Ejecutivo de interacción").JavaDialog("Resumen de la orden (Orden").Exist Then
		...
		wait 2
		Dim x
		x = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").JavaCheckBox("El cliente firmó.").GetROProperty("enabled")
		If x <> "0" Then
...
Sub EmpujeOrden()
		...
		If DataTable("e_Tipo_De_DATA_Sim", dtLocalSheet) = "DATA LOGICA" Then
			...
			Wend
			wait 2
			JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Grupo de órdenes_3").JavaList("Estado de la gestión manual:").Select "Cumplimiento Completo Parcial"
			wait 2
