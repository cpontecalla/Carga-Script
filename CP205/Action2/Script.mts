	...
	If ((ucase(Motivo) = "CAEQ_EQUIPO") or (ucase(Motivo) = "CAEQ_CAPL")) Then
		...
		Select Case ucase(Equipo)
			...
			Case "HUAWEI Y8S NEGRO JKM-LX3"
				wait 5
				While JavaWindow("Ejecutivo de interacción").JavaDialog("Cambio Simplificado (Para").JavaCheckBox("Seleccionar_3").Exist = false
					wait 1
				Wend
				wait 1
				JavaWindow("Ejecutivo de interacción").JavaDialog("Cambio Simplificado (Para").JavaCheckBox("Seleccionar_3").Set "ON"
				JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "Dispos.png", True
				imagenToWord "Dispositivo seleccionado",RutaEvidencias() & "Dispos.png"
...
RunAction "WIC_2", oneIteration
	...
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").JavaButton("Enviar orden").Click @@ hightlight id_;_12430702_;_script infofile_;_ZIP::ssf2.xml_;_
		...
		t=0
		While (JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Orden 836795A").JavaEdit("TextAreaNative$1").Exist or JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaTable("SearchJTable").Exist)= False
			Wait 1
			t = t + 1
			If (t >= 180) Then
				DataTable("s_Resultado", dtlocalSheet) = "Fallido"
	...
	If JavaWindow("Ejecutivo de interacción").JavaDialog("Mensajes de validación").JavaTable("SearchJTable").Exist = True Then
		wait 1
		JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &"MensajePenalidad.png", True
		imagenToWord "Mensaje de validación",RutaEvidencias() &"MensajePenalidad.png"
...
Sub EnviarPago()
	...
	imagenToWord "Envio de Pago",RutaEvidencias() & "Pago.png"
