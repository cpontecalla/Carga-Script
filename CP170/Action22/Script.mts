...
Sub Portabilidad()
	...
	If ucase(DataTable("Tipo_Servicio", dtLocalSheet)) = "MOVIL" Then
		Set shell = CreateObject("Wscript.Shell") 
		shell.SendKeys "{DOWN 1}"
		shell.SendKeys "{ENTER}"
	ElseIf ucase(DataTable("Tipo_Servicio", dtLocalSheet)) = "FIJA" Then
		Set shell = CreateObject("Wscript.Shell") 
	...
	Wend
	wait 1
	If  ucase(DataTable("Tipo_Venta", dtLocalSheet)) = "SOLO LINEA" Then
	...
	Window("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "Portabilidad.png", True
	imagenToWord "Portabilidad: Datos a consultar", RutaEvidencias() & "Portabilidad.png"
	Window("Ejecutivo de interacción").InsightObject("InsightObject_15").Click
	While (Window("Ejecutivo de interacción").InsightObject("InsightObject_28").Exist or Window("Ejecutivo de interacción").InsightObject("InsightObject_31").Exist)= False
		wait 1
	Wend
	wait 1
	If Window("Ejecutivo de interacción").InsightObject("InsightObject_31").Exist = true Then
		Window("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "ErrorPorta.png", True
		imagenToWord "Consulta rechazada", RutaEvidencias() & "ErrorPorta.png"
		ExitTestIteration
	End If
	wait 1
	Set shell = CreateObject("Wscript.Shell") 
	shell.SendKeys "{PGDN  2}"
	Window("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "Portabilidad2.png", True
	imagenToWord "Consulta aprobada", RutaEvidencias() & "Portabilidad2.png"
	wait 1
	While Window("Ejecutivo de interacción").InsightObject("InsightObject_29").Exist = false
	 	wait 1
	 Wend
	 wait 1
	Window("Ejecutivo de interacción").InsightObject("InsightObject_29").Click
	While  (Window("Ejecutivo de interacción").InsightObject("InsightObject_30").Exist or Window("Ejecutivo de interacción").InsightObject("InsightObject_24").Exist) = false
		wait 1
	Wend
	wait 2
	If Window("Ejecutivo de interacción").InsightObject("InsightObject_24").Exist = true Then
		...
		ExitTestIteration
	End If
	If Window("Ejecutivo de interacción").InsightObject("InsightObject_30").Exist = True Then
		Window("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "ScoreCalculado.png", True
		imagenToWord "Score Calculado", RutaEvidencias() & "ScoreCalculado.png"
		Set shell = CreateObject("Wscript.Shell") 
		shell.SendKeys "{PGDN 2}"
