...
Sub Porta()
	...
	While t = ""
		...
		t = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Formulario de portabilidad").JavaEdit("Nombre del cliente:").GetROProperty("text")
	Wend
	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Formulario de portabilidad").JavaList("Tipo de Cliente:").Select "Individuo"
	wait 1
...
'				
		...
		wait 1
				...
				If JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Distribución").Exist Then
					...
					Wend
					Dim q
					q = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Distribución").JavaEdit("Nombre y Dirección de").GetROProperty("text")
					While q = ""
						wait 1
						q = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Negociar Distribución").JavaEdit("Nombre y Dirección de").GetROProperty("text")
					Wend
...
'	imagenToWord "Resumen Orden", RutaEvidencias() &"ResumenOrden.png"
'	Dim h
'	h = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").GetROProperty("text")
'	DataTable("s_Nro_Orden", dtLocalSheet) = h
'	JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").Close
'	wait 1
'	While JavaWindow("Ejecutivo de interacción").JavaDialog("Cerrar negociación de").JavaButton("Guardar").Exist = False
'		wait 1
'	Wend
'	wait 1
...
'	ExitActionIteration
	If JavaWindow("Ejecutivo de interacción").JavaDialog("Resumen de la orden (Orden").Exist(1) Then
		JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "GenerarContrato.png", True
		imagenToWord "Link de documentación", RutaEvidencias() & "GenerarContrato.png"
