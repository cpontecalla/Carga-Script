...
Sub Financiamiento()
	...
	wait 1
		...
		Wend
		Dim jh
		jh = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaEdit("BAR ID").GetROProperty("text")
		While jh = ""
			wait 1
			jh = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Pago Inmediato").JavaEdit("BAR ID").GetROProperty("text")
		Wend
	...
	For Iterator = 0 To Count-1 step 1
		...
		If rs = DataTable("e_MedioPago", dtLocalSheet) Then
			    ...
			    wait 1
			    If DataTable("e_MedioPago", dtLocalSheet) = "Pago a la Factura" Then
...
'					imagenToWord "MedioPago", RutaEvidencias() &Num_Iter&"_"&"Financiamiento"&".png"
...
Sub CodigoVendedor()
    ...
    wait 1
    JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "Vendedor.png", True
	imagenToWord "Seleccionamos vendedor",RutaEvidencias() & "Vendedor.png"
...
'		wait 1
		...
		While(JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Grupo de órdenes_2").JavaButton("Enviar").Exist or JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Grupo de órdenes").JavaButton("Validar y Crear Factura").Exist) = False
			wait  1
		Wend
		If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Grupo de órdenes").JavaButton("Validar y Crear Factura").Exist = True Then
		...
		JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() & "Pago.png", True
		imagenToWord "Enviamos Pago",RutaEvidencias() & "Pago.png"
