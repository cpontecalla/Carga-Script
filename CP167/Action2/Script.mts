...
Sub CambioSimplificado()
 ...
 @@ hightlight id_;_29628800_;_script infofile_;_ZIP::ssf8.xml_;_
	If ((ucase(Motivo) = "CAEQ_EQUIPO") or (ucase(Motivo) = "CAEQ_CAPL")) Then
			...
			While (JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Cambio Simplificado (Para").JavaStaticText("Su búsqueda devolvió una").Exist or JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Cambio Simplificado (Para").JavaCheckBox("Seleccionar").Exist )= False
				wait 1
			Wend
...
Sub ResumenOrden()
	...
	End If
		...
		While JavaWindow("Ejecutivo de interacción").JavaDialog("Resumen de la orden (Orden").JavaTable("SearchJTable").Exist = False
			...
			If (t >= 180) Then
				...
				DataTable("s_Detalle", dtlocalSheet) = "Error en la Carga de la negociación del Pago"
				JavaWindow("Ejecutivo de interacción").CaptureBitmap RutaEvidencias() &Num_Iter&"_"&"ErrNegociarPago.png", True
				imagenToWord "Error en la Carga Link de documentación",RutaEvidencias() &Num_Iter&"_"&"ErrNegociarPago.png"
				Reporter.ReportEvent micFail, DataTable("s_Resultado", dtlocalSheet), DataTable("s_Detalle", dtlocalSheet)
				Wait 2
				ExitActionIteration
			End If	
	...
	imagenToWord "Link de documentación",RutaEvidencias() &"Link.png"
	JavaWindow("Ejecutivo de interacción").JavaDialog("Resumen de la orden (Orden").Close
	wait 3
	Dim enabled
	enabled = JavaWindow("Ejecutivo de interacción").JavaInternalFrame("Resumen de la orden (Orden").JavaCheckBox("El cliente firmó.").GetROProperty("enabled")
	If enabled = 1 Then
					...
					Loop While Not((JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").Exist) Or (varhab="1"))
						...
						If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").Exist(1) Then
							If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").Exist(0) Then
								If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Error externo: Fall??").Exist = True Then
									varlog = JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Error externo: Fall??").GetROProperty("text")
								End If
								If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Longitud del ID de equipo:").Exist = True Then
									varlog = JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Longitud del ID de equipo:").GetROProperty("text")
								End If
								If JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Por favor rellenar todas").Exist = True Then
									varlog = JavaWindow("Ejecutivo de interacción").JavaDialog("Buscar: Orden > Solicitar").JavaDialog("Mensaje").JavaStaticText("Por favor rellenar todas").GetROProperty("text")
								End If
